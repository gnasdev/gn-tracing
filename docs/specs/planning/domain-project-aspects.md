...
| 24. Release and install packaging | Build emits unpacked extension; release zip is for manual install; Store packaging is separate from GitHub release assets. | `Taskfile.yml`, `scripts/check-store-package.mjs` | `features/release-and-update-checks.md` | Sync versions and clarify Store vs GitHub release flows. |
...
| 26. Build and developer tooling | Root esbuild owns extension; standalone Vite owns hosted player; Taskfile wraps build/check/deploy; Biome handles supported source formatting/lint while Markdown docs use a dedicated hygiene check. | `package.json`, `esbuild.config.mjs`, `Taskfile.yml`, `player-standalone/vite.config.ts` | `_index.md`, `project-context.md`, `drive-and-player.md` | Add developer docs only if user wants onboarding beyond domain docs. |
