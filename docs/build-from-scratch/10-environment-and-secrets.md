...
`player/deploy.sh` and `worker/deploy.sh` read these from the **process environment only** (they do not source repository `.env`). Export them in the shell or CI before `task player:deploy` / `task worker:deploy`.
