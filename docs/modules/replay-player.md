...
- network resource filters that follow DevTools-like buckets (`fetch`/`js`/`css`/…): canonical CDP `resourceType` maps 1–1 (XHR/Fetch stay under Fetch/XHR even when the URL looks like `.js`); only missing/`Other` types are refined from MIME/URL; `.map` is not JS. Classifier source of truth: `src/shared/network-filter-type.ts` via `window.gnCore.network`
- source-mapped console locations, parsed Error argument stacks, bounded source snippets, and source-map diagnostic messages
- network request/response details, headers, body text (or a clear empty-state when no body was stored), cURL copy, response previews for HTML/media, syntax-highlighted source views, and JSON pretty preview when validation succeeds
...
- when Instant Replay (or other DOM snapshots) is present **without video and without annotated stills**, a structural **DOM stage** in the media column: sandboxed iframe preview rebuilt from `DomNode` trees (`gnCore.dom.hydrateDomNodeToHtml`) plus a frame scrubber (prev/next/range) synced to the Elements snapshot select. This is structural HTML lookback (inline attributes + optional `<base href>`), not full CSSOM/rrweb pixel fidelity — external stylesheets may not apply under the empty iframe `sandbox`
