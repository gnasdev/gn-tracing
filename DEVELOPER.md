...
- Biome owns formatting, linting, and import organization for its supported source types. Markdown docs are covered by `npm run docs:check`. Format, lint, and unit-test gates run via **Husky**, not CI: `pre-commit` is fast (Biome staged + docs + version:check + vitest related); `pre-push` runs the full gate (`typecheck:all`, `check`, `test:all`). GitHub Actions covers the rest: `.github/workflows/test.yml` runs `task typecheck:all` plus a `task dist:all` production build of the Chrome, Edge, Opera, and Firefox packages on push to `main`/`dev` and on every PR, and `.github/workflows/release.yml` runs on `v*` tags to build and publish the extension zip to a GitHub Release. Neither workflow runs lint or unit tests, so a `SKIP_HOOKS=1` push bypasses those entirely. Hooks **never** deploy Worker or Player. Scripts: `npm run hooks:pre-commit`, `npm run quality:gate`. Skip push gate with `SKIP_HOOKS=1 git push`. Optional player e2e: `RUN_E2E=1 git push`.
...
Player and OAuth Worker production releases are versioned and immutable. Player artifacts are stored in R2 and served by `gn-tracing-player-router`; versioned Worker services are reached through `gn-tracing-oauth-proxy`. Publish them with `task player:release:upload` and `task edge:player:deploy` / `task edge:worker:deploy` — never as part of a commit or push.
