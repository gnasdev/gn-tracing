...
function updateUI(status) {
  ...
  if (status.isRecording) {
    ...
    consoleCount.textContent = status.consoleLogCount || 0;
    networkCount.textContent = status.networkRequestCount || 0;
  } else {
...
toggleBtn.addEventListener("click", async () => {
  ...
  if (status.isRecording) {
    ...
    const result = await chrome.runtime.sendMessage({ action: "START_RECORDING", tabId: tab.id });
...
downloadBtn.addEventListener("click", async () => {
  ...
  try {
    const result = await chrome.runtime.sendMessage({ action: "DOWNLOAD_RESULTS" });
...
uploadBtn.addEventListener("click", async () => {
  ...
  try {
    const result = await chrome.runtime.sendMessage({ action: "UPLOAD_RECORDING" });
    if (result.ok) {
...
chrome.runtime.sendMessage({ action: "GET_SERVER_URL" }).then((res) => {
