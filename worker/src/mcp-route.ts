...
const MAX_REMOTE_PACKAGE_BYTES = 24 * 1024 * 1024;
/** One JSON artifact this large would blow the response budget anyway. */
const MAX_REMOTE_ENTRY_BYTES = 8 * 1024 * 1024;
/** Per-IP hourly call budget. An investigation is ~10 calls. */
...
export interface McpEnv {
  /** Set to "false" to disable the remote MCP endpoint on a deployment. */
  MCP_ENABLED?: string;
  /** Origin of the hosted player, whose /api/* proxies stream package bytes. */
...
export function isMcpEnabled(env: McpEnv): boolean {
...
export async function handleMcpRequest(request: Request, env: McpEnv): Promise<Response> {
  ...
  if (!(await consumeMcpRateLimit(request)).allowed) {
    return jsonRpcError(
      -32000,
...
export function createRemoteRecordingStore(env: McpEnv) {
...
export { MAX_REMOTE_ENTRY_BYTES, MAX_REMOTE_PACKAGE_BYTES };

/**
 * Per-IP hourly rate limit via the Cache API, mirroring the feedback limiter.
 * Best-effort across colos; enough to stop casual abuse of a public endpoint.
 */
async function consumeMcpRateLimit(request: Request): Promise<{ allowed: boolean }> {
  const ip =
    request.headers.get("CF-Connecting-IP") ||
    request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ||
    "unknown";
  const hourBucket = Math.floor(Date.now() / MCP_RATE_WINDOW_MS);
  const keyHash = await hashKey(`${hourBucket}:${ip}`);
  const cacheKey = new Request(`https://mcp-rate.gn-tracing.local/${keyHash}`);
    ...
    await cache.put(
      ...
      new Response(String(count + 1), {
        headers: {
          ...
          "Cache-Control": `max-age=${Math.ceil(MCP_RATE_WINDOW_MS / 1000)}`,
        },
      }),
    );
    return { allowed: true };
  } catch {
    // No Cache API (local dev): allow rather than break the endpoint.
    return { allowed: true };
  }
}

/** Hashes the IP so the rate-limit key never stores a raw address. */
