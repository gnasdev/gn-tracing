/**
 * macOS Safari full-record path: in-page evidence + extension-page display video.
 *
 * Same evidence/media model as Firefox — Safari has no chrome.debugger/CDP
 * equivalent either, so this follows the same in-page-console + webRequest-network
 * + getDisplayMedia composition. Kept as a separate class rather than a
 * generalized `FirefoxRecordingRuntime` because the popup-window/gesture
 * behavior of `getDisplayMedia` on Safari has not been validated against
 * Firefox's assumptions yet (see plan risk register); once proven identical
 * in practice, the two classes are candidates to merge.
 *
...
export class SafariRecordingRuntime implements RecordingRuntime {
   ...
   *
   * A navigation destroys the injected content scripts, same as on Firefox.
   */
  ...
  async finalizeEvidence(_input: RecordingFinalizeInput): Promise<RecordingFinalizeResult> {
    ...
    // Safari, like Firefox, cannot capture a single tab, so the video always
    // shows more than the recorded page. Say which surface was shared instead
    // of letting the viewer assume a tab-scoped recording.
  ...
  async captureDomSnapshotMarker(_label: string): Promise<void> {
    // No CDP DOM snapshots on Safari full record.
  }
}
