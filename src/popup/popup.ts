 ...
 * via getDisplayMedia (same user gesture) — no intermediate "Choose what to
 * share" panel. This function waits for the stream, parks the media host only
 * after the picker is live, hands off tracks, then START_RECORDING with
 * mediaPrearmed. Fallback: normal START_RECORDING (media-host auto-arm, then
 * tab-frame).
 *
 * Safari does NOT use this path (tried in a prior revision, reverted): its
 * toolbar-popup is a transient NSPopover that Safari auto-closes as soon as
 * the OS share picker steals focus, killing the JS context (and the pending
 * getDisplayMedia promise) before it can resolve — Start silently did
 * nothing, no error, no fallback. Safari always goes through
 * runRecordingStartPreflight() + plain START_RECORDING below, which auto-arms
 * via a real chrome.windows.create() window (not a popover), same as before
 * Firefox got this popup-gesture optimization.
 */
...
toggleBtn.addEventListener("click", async () => {
  ...
  //
  // Safari is deliberately excluded: calling getDisplayMedia() from inside
  // this toolbar popup causes Safari to close the popup (its NSPopover loses
  // focus to the OS share picker), killing the pending promise before it can
  // resolve — Start silently did nothing. Safari uses the preflight branch
  // below instead, which lets the arm-window mechanism (a real window, not a
  // popover) drive getDisplayMedia.
